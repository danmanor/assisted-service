...
def deploy_wiremock(deploy_options: Namespace):
    ...
    if deploy_options.target == "kind":
        log.info("Deploying Wiremock Ingress")
        deploy_ingress(
            hostname=socket.gethostname(),
            deploy_options=deploy_options
        )
        log.info("Wiremock Ingress deployment completed")
        hostname = socket.gethostname()
        port="80"
        ...
        hostname = get_service_external_ip(deploy_options=deploy_options.namespace)
...
def is_wiremock_service_ready(namespace: str) -> bool:
    return get_service_external_ip(namespace=namespace) != ""
